...
class conroy:
    '''
    The Conroy et al. (2010) attenuation law was derived for local disk 
    ...
    def __init__(self, EBV=0.15, EBV_lims=[-0.05, 2.00], EBV_delta=0.02,
        ...
        EBV : float
            ...
            Relates to Av, the magnitude of extinction in the V band (5500A) via
                Av = Rv * EBV
        B : float (held fixed throughout the fitting)
    ...
    def conroylaw(self, wave):
        ...
        sel0 = np.nonzero(np.logical_and(x >= 8.0, x < 10.0))[0]
        ...
        a0, b0 = np.zeros(sel0.shape), np.zeros(sel0.shape)
        ...
        x0 = x[sel0]
        y = x0 - 8.
        a0 = -1.073 - 0.628*y + 0.137*y**2. - 0.070*y**3.
        b0 = 13.670 + 4.257*y - 0.420*y**2. + 0.374*y**3.
    ...
    def evaluate(self, wave, new_wave=False):
        ...
        if self.axlam is None:
            axlam, bxlam = self.conroylaw(wave)
            self.axlam = axlam
            self.bxlam = bxlam
        if new_wave:
            axlam, bxlam = self.conroylaw(wave)
...
class cardelli:
    ...
    def cardellilaw(self, wave):
        ''' Cardelli et al. (1989) extinction curve, a(x) and b(x)
        where A(wave) / Av = a(x) + b(x) / Rv
        and x = 1 / wave (units inverse microns)
    ...
    def evaluate(self, wave, new_wave=False):
        ...
        if self.axlam is None:
            axlam, bxlam = self.cardellilaw(wave)
            self.axlam = axlam
            self.bxlam = bxlam
        if new_wave:
            axlam, bxlam = self.cardellilaw(wave)
